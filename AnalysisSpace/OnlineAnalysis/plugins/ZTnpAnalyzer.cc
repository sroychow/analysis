...
ZTnpAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //std::cout << "Loose mu size=" << looseSIPMuFSRpair->size() <<std::endl;  
  //std::cout << "ZTnP::"
...
void ZTnpAnalyzer::fillZmumuTnp(const edm::Handle<MuFSRCollection>& looseSIPMuFSRpair, const edm::Handle<pat::PackedCandidateCollection>& fsr,const reco::Vertex& vit) {
  for(unsigned int ti = 0; ti < looseSIPMuFSRpair->size(); ti++) {
    ...
    //trigger matched 
    //if( tag.first.triggerObjectMatchByPath("HLT_IsoMu20_v*",true) == nullptr 
    //    && tag.first.triggerObjectMatchByPath("HLT_IsoTrkMu20_v*",true) == nullptr 
    //    && tag.first.triggerObjectMatchByPath("HLT_IsoMu22_v*",true) == nullptr 
    //    && tag.first.triggerObjectMatchByPath("HLT_IsoTrkMu22_v*",true) == nullptr )     continue;
   
    //std::cout << "Tag passed Trigger!!" << std::endl;
